...
export function HttpNode({ data }: NodeProps<HttpNodeData>) {
  const nodeColor = getNodeColor('http');
  const icon = getNodeIcon('http');
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="flex-1">
          <div className="text-xs font-semibold text-gray-500 uppercase">HTTP Request</div>
          ...
          {data.parameters.method && (
            <div className="text-xs">
              <span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 rounded font-semibold">
                {data.parameters.method.toUpperCase()}
              </span>
            </div>
          )}
          {data.parameters.url && (
            <div className="text-xs text-gray-600 truncate">{data.parameters.url}</div>
          )}
        </div>
      )}
