...
export function AINode({ data }: NodeProps<AINodeData>) {
  const nodeColor = getNodeColor('ai');
  const icon = getNodeIcon('ai');
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="flex-1">
          <div className="text-xs font-semibold text-gray-500 uppercase">AI / LLM</div>
      ...
      {data.hasError && data.error && (
        <div className="mt-2 p-2 bg-red-50 rounded text-xs text-red-700">
          {data.error.message || 'AI execution failed'}
        </div>
      )}
