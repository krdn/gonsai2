...
```
[192.168.0.8 로컬 PC] --SSH(포트 2222)--> [192.168.0.5 서버]
     VSCode Remote                          개발 서버 실행
```
...
| 환경     | 서비스     | 포트    | 바인딩  |
| -------- | ---------- | ------- | ------- |
| **개발** | 프론트엔드 | 3002    | 0.0.0.0 |
| **개발** | 백엔드 API | 3000    | 0.0.0.0 |
| **개발** | WebSocket  | 3000/ws | 0.0.0.0 |
| **운영** | 프론트엔드 | Docker  | -       |
| **운영** | 백엔드     | Docker  | -       |
...
| 서비스     | 개발 환경 URL                      |
| ---------- | ---------------------------------- |
| 프론트엔드 | `http://192.168.0.5:3002`          |
| 백엔드 API | `http://192.168.0.5:3000`          |
| API 문서   | `http://192.168.0.5:3000/api-docs` |
| WebSocket  | `ws://192.168.0.5:3000/ws`         |

### 방화벽 설정 (iptables)

개발 서버 외부 접속을 위해 열어둔 포트:

- 3000 (백엔드)
- 3001 (WebSocket - 레거시)
- 3002 (프론트엔드)

설정 파일: `/etc/iptables.rules`
...
> ⚠️ 프론트엔드는 반드시 `-H 0.0.0.0` 옵션으로 실행해야 외부 접속 가능

### 환경 변수 파일

- **Backend**: `apps/backend/.env` (MongoDB URI, n8n API 키)
