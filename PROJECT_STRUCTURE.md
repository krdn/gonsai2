...
> n8n 통합 최적화 프로젝트 - AI 협업에 최적화된 구조
...
├── connection-test.log            # Docker 환경 테스트 결과
├── docker-compose.override.yml    # Docker 네트워크 연동
│
├── .ai/                           # AI 컨텍스트 디렉토리
│   ├── context-map.json           # 워크플로우 컨텍스트 맵
│   ├── error-patterns.json        # 오류 패턴 지식 베이스
│   ├── n8n-templates/             # 재사용 가능한 워크플로우 템플릿
│   ├── workflow-docs/             # 워크플로우별 상세 문서
│   └── README.md                  # AI 컨텍스트 가이드
│
└── features/                      # 기능 모듈 (모노레포 스타일)
    ├── n8n-integration/           # n8n REST API 통합
    │   ├── types.ts               # TypeScript 타입 정의
    │   ├── api-client.ts          # n8n API 클라이언트
    │   ├── webhook-handler.ts     # 웹훅 요청 처리
    │   ├── workflow-executor.ts   # 워크플로우 실행 관리
    │   ├── workflow-monitor.ts    # 실행 모니터링
    │   └── README.md              # 모듈 문서
    │
    ├── agent-orchestration/       # AI Agent 작업 조율
    │   ├── types.ts               # TypeScript 타입 정의
    │   ├── agent-manager.ts       # Agent 생명주기 관리
    │   ├── execution-queue.ts     # 우선순위 기반 작업 큐
    │   ├── result-processor.ts    # 실행 결과 처리
    │   └── README.md              # 모듈 문서
    │
    └── error-healing/             # 자동 오류 진단 및 치유
        ├── types.ts               # TypeScript 타입 정의
        ├── n8n-error-analyzer.ts  # n8n 특화 오류 분석
        ├── workflow-fixer.ts      # 워크플로우 자동 수정
        └── README.md              # 모듈 문서
```
...
**설계 원칙**: Kent Beck의 Augmented Coding 원칙 적용

#### 1. n8n-integration/ (n8n REST API 통합)
...
#### 2. agent-orchestration/ (AI Agent 작업 조율)
...
gonsai2/
├── features/
│   ├── workflow-builder/       # AI 기반 워크플로우 빌더
│   │   ├── template-engine.ts
│   │   └── node-recommender.ts
│   │
...
```
gonsai2/
├── packages/                   # Monorepo 전환
│   ├── core/                   # 공통 핵심 로직
│   ├── n8n-client/             # n8n 클라이언트 패키지
│   ├── database/               # 데이터베이스 패키지
│   └── api/                    # REST API 서버
│
└── apps/                       # 애플리케이션
...
## 🎓 Kent Beck 원칙 적용 체크리스트

### ✅ 명확한 의도 (Clear Intent)

- [x] 모든 함수와 클래스 이름이 목적을 명확히 표현
- [x] 파일 이름이 내용을 정확히 반영
- [x] 디렉토리 구조가 기능별로 명확히 분리
...
- [x] `.ai/` 디렉토리에 구조화된 컨텍스트
- [x] 모든 주요 함수에 `@aiContext` 주석
- [x] README.md에 사용 예시 포함
...
- **AI 컨텍스트**: [.ai/README.md](./.ai/README.md)
- **Docker 연동**: [connection-test.log](./connection-test.log)
